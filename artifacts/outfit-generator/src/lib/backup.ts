 ...
 * and imports it back.
 *
 * Format: JSON file (collection-backup-<date>.json) containing clothing
...
async function exportNative(json: string, filename: string): Promise<void> {
  try {
    ...
    const path = `collection-exports/${filename}`;
    ...
    await Share.share({
      ...
      url: uri,
      dialogTitle: 'Save or share your collection backup',
    });
  } catch (err) {
